...
- **overlay_theme**: `"jarvis"`, `"glass"`, `"sakura"`, `"warcraft"`, or omit for the default overlay — macOS only (default: none)
  ...
  - **warcraft**: Warcraft 3 command card with gold border, pack portrait, and animated GIF. Per-pack themed headers and flavor quotes (e.g. "JOB'S DONE", "SOMETHING NEED DOING?")
...
- **notification_all_screens** (boolean, default: `true`): Show overlay notifications on all screens (`true`) or only the main screen (`false`). Themed overlays (`glass`, `jarvis`, `sakura`, `warcraft`) previously only showed on one screen — existing configs with those themes are migrated to `false` automatically. macOS only.
