...
- **overlay_theme**: `"jarvis"`, `"glass"`, `"sakura"`, `"warcraft"`, 또는 기본 오버레이 사용 시 생략 — macOS 전용 (기본값: 없음)
  - **jarvis**: 회전하는 아크, 눈금, 진행률 링이 있는 원형 HUD
  - **glass**: 강조 색상 바, 진행선, 타임스탬프가 있는 글래스모피즘 패널
  - **sakura**: 분재 나무와 애니메이션 벚꽃 꽃잎이 있는 선 정원
  - **warcraft**: Warcraft 3 커맨드 카드 스타일. 금색 테두리, 팩 초상화, 애니메이션 GIF. 팩별 테마 헤더와 대사 (예: "JOB'S DONE", "SOMETHING NEED DOING?")
